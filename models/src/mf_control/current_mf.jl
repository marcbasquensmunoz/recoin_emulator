...
for i in 1:Nt-Nt_train
    ...
    μ_T = mean(input_prediction_ahead, dims=2)[1:3]
    σ_T = std(input_prediction_ahead, dims=2)[1:3]
    input_control = mf_original[i+Nt_train]
...
t_axis_truth = (1:Nt) ./ 24
t_axis_pred = (Nt_train+1:Nt) ./ 24
ax_Tin = Axis(fig_example[1, 1], xlabel = L"t \ (days)", ylabel = L"T_{in} \ (K)", title = "Prediction of the next hour")
lines!(ax_Tin, t_axis_truth, Tin_original[1,:], label="Truth")
lines!(ax_Tin, t_axis_pred, prediction[1,:], label="Prediction")

ax_Tb = Axis(fig_example[2, 1], xlabel = L"t \ (days)", ylabel = L"T_{b} \ (K)")
lines!(ax_Tb, t_axis_truth, Tb_original[1,:], label="Truth")
lines!(ax_Tb, t_axis_pred, prediction[2,:], label="Prediction")

axislegend(""; position= :rt, backgroundcolor = (:grey90, 0.25));

ax_Q = Axis(fig_example[3, 1], xlabel = L"t \ (days)", ylabel = L"Q \ (W/m)")
lines!(ax_Q, t_axis_truth, Q_original[1,:], label="Truth")
...
# Save the figure
save("current_mf_example_3.png", fig_example, px_per_unit=2.0)
...
ax_Tin = Axis(fig_example[1, 1], xlabel = L"t \ (days)", ylabel = L"T_{in} \ (K)", title = "Prediction of the next hour")
lines!(ax_Tin, t_axis_truth, Tin_original[1,:], label="Truth")
lines!(ax_Tin, t_axis_pred, prediction_ahead[1,Nt_train+1:end], label="Prediction")

ax_Tb = Axis(fig_example[2, 1], xlabel = L"t \ (days)", ylabel = L"T_{b} \ (K)")
lines!(ax_Tb, t_axis_truth, Tb_original[1,:], label="Truth")
lines!(ax_Tb, t_axis_pred, prediction_ahead[2,Nt_train+1:end], label="Prediction")

axislegend(""; position= :rb, backgroundcolor = (:grey90, 0.25));

ax_Q = Axis(fig_example[3, 1], xlabel = L"t \ (days)", ylabel = L"Q \ (W/m)")
lines!(ax_Q, t_axis_truth, Q_original[1,:], label="Truth")
lines!(ax_Q, t_axis_pred, prediction_ahead[3,Nt_train+1:end], label="Prediction")
...
# Save the figure
save("current_mf_recursive_example_1.png", fig_example, px_per_unit=2.0)
