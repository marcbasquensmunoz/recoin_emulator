...
for i in 1:N
    ...
    mf = abs.(lsq_denoise(rand(Uniform(0., 1.), Nt),  order=2) .+ rand(Uniform(-0.4, 0.4)))
    ...
    @views @. Tin_synth[:, i] = containers.X[1, :]
    @views @. Tb_synth[:, i] = containers.X[5, :]
    @views @. Q_synth[:, i] = containers.X[7, :]
    @views @. mf_synth[:, i] = mf
end
