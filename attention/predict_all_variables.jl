...
model_save_file = "$(@__DIR__)/predict_all_variables.jld2"
...
X = zeros(Float32, 3, Nt_train, N_train) # Input 
...
model = Chain(
    Dense(3 => hidden_size),
    MultiHeadAttention(hidden_size => hidden_size_2 => hidden_size_2, nheads = 2),
    x -> x[1],
    Dense(hidden_size_2 => 3)
) 
...
input_full = zeros(3, Nt, 1)
input_full[:, :, 1] = Float32.(vcat(Tin_original, Tb_original, Q_original))
...
for i in 1:Nt-Nt_train
    ...
    μ_input = mean(input, dims=2)
    σ_input = std(input, dims=2)
